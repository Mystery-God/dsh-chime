...
当前会话的 agent 任务结束时播放提示音（叮咚），支持音量调节、静音、更换内置音效、上传自定义音频文件。设置页位于「设置 → 插件 → 任务完成提示音」，另带一个左下角悬浮音量控件。

A task-completion chime plugin for the DeepSeek Harness Web GUI: rings a ding-dong when the current session's agent turn finishes, with volume control, mute, built-in sound presets, and custom-audio upload. Settings live at **Settings → Plugins → 任务完成提示音**, plus a floating volume control in the bottom-left corner.
...
- 🎚️ **悬浮音量控件 · Floating volume control**（左下角）：静音开关、音量滑块、试听按钮，改动即持久化。Mute toggle, volume slider, preview button; every change persists immediately.
...
lib/client.js  — 浏览器半体：设置页（settings.plugins.tab）+ 悬浮控件（shell.overlay）+ 完成监听（useSessions 标准 props）
...
- 音量/静音/音效选择即时 PUT 到 host 持久化；浏览器内存中共享一份 store，设置页与悬浮条实时同步。Changes PUT to the host immediately; both UI surfaces share one in-memory store and stay in sync live.
