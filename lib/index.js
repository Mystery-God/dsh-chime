 ...
 *
 * The browser half (./client) renders the Plugins-settings tab. The
 * completion trigger lives entirely in the browser (session `running` flag
 * transitions), so the host exposes no websocket or polling surface for it.
 */
 ...
 */
export const CHIME_GUIDANCE = '本机已安装 dsh-chime 插件（任务完成提示音）：任务完成时（含后台任务）播放提示音；设置页在「设置 → 插件 → 任务完成提示音」，可调音量、静音、更换内置音效或上传自定义音频文件。自定义音频存于 ~/.dsh/chime/audio，上限 16MB。用户提到「提示音 / 叮咚 / 任务完成声音」时即指本插件，请据此协作。'
